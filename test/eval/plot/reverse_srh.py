...
from eval.utils import FLOWBENDER_MEASUREMENT_TIME, LINE_WIDTH, MARKER_SIZE, FONTSIZE, LOAD_BALANCER_MEASUREMENT_TIME
...
def bw_over_load_balancer(db, output_path):

    # Take the last working experiment of reverse SRH with load balancer avoidance after three-way handshake
    exp = db.query(TCPeBPFExperiment) \
        .filter_by(valid=True, failed=False, random_strategy="reverse_srh_load_balancer") \
        .order_by(TCPeBPFExperiment.timestamp.desc())[0]

    figure_name = "bw_over_load_balancer_{topo}_{demands}" \
        .format(topo=os.path.basename(exp.topology), demands=os.path.basename(exp.demands))
    fig = plt.figure()
    subplot = fig.add_subplot(111)
    ...
    subplot.set_ylabel("Bandwidth (Mbps)", fontsize=FONTSIZE)
    subplot.set_ylim(bottom=0)
    subplot.set_xlim(left=1, right=LOAD_BALANCER_MEASUREMENT_TIME)

    lg.info("Saving figure of bw_over_load_balancer for '{topo}' demand '{demands}' to {path}\n"
            .format(topo=os.path.basename(exp.topology), demands=os.path.basename(exp.demands),
                    path=os.path.join(output_path, figure_name + ".pdf")))
    fig.savefig(os.path.join(output_path, figure_name + ".pdf"), bbox_inches='tight', pad_inches=0, markersize=9)
    plt.clf()
