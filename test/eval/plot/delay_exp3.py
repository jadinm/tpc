...
def plot_aggregated_ab_cdfs(delay_experiments: List[ShortTCPeBPFExperiment], output_path, hotnet_paper=False):
    ...
    for exp in delay_experiments:
        ...
        gamma = "ECMP" if not exp.ebpf else f"Γ={exp.gamma_value}"
        key = f"{gamma}, {parallel_connections} queries in parallel, {volume}kB, {delay_factor} times"
        if len(to_aggregate.get(key, [])) >= 10:  # Only take the most recent experiment
            continue
    ...
    groups = {
        ...
        "gamma_10_conn_var": lambda x: "Γ=0.1" in x and "100kB" in x and "10 times" in x,
    ...
    for key, exp_list in to_aggregate.items():
        ...
        latencies = []
        for exp in exp_list:
            for latency in exp.abs.first().ab_latency.all():
                latencies.append(latency.latency / 10 ** 3)  # ms
        print(key)
        print(len(latencies))

        bins, cdf = cdf_data(latencies)

        for group, subplot in subplots.items():
            if groups[group](key):  # Filter experiences
                # TODO Change color
                subplot.step(bins, cdf, linewidth=LINE_WIDTH,
                             where="post", markersize=MARKER_SIZE, label=labels[group](key))
        ...
        for exp in exp_list:
            ...
            for db_snap in exp.snapshots.all():
                ...
                if first_sequence == -1:
                    ...
                    start_time = snap.time
                rel_time = (snap.time - start_time) / 10 ** 9  # in seconds
                weights_over_time.append((rel_time, snap.weights))

            weights_over_time.sort()
            nbr_paths = 2  # TODO Recover from topo ?
            first_convergence = -1
            second_convergence = -1
            convergence_limit = 0.9
            failure_time = int(json.loads(exp.tc_changes)[0][
            ...
            if first_convergence > 0:
                first_convergence_data.append(first_convergence)
                first_convergence_tries_data.append(second_convergence_tries)
            if second_convergence > 0:
                second_convergence_data.append(second_convergence)
                second_convergence_tries_data.append(second_convergence_tries)
            print(first_convergence_tries, second_convergence_tries)

        # Plot 1st and 2nd convergence times and tries
        for graph_group, graph_subplots in [(first_convergence_data, subplots_convergence),
                                            (second_convergence_data, subplots_convergence_2),
                                            (first_convergence_tries_data, subplots_convergence_tries),
                                            (second_convergence_tries_data, subplots_convergence_tries_2)]:
